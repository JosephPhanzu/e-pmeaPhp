<?php
    ...
    $outPut = "";
    if (isset($_POST['r']) && isset($_POST['type']) && isset($_POST['id'])) {
        $rech = securisation($_POST['r']);
        $type = securisation($_POST['type']);
        $getId = securisation(htmlspecialchars(intval($_POST['id'])));

        $req = "SELECT * FROM patentecom WHERE typePat= ? AND id_user = ? AND concat(nom,nature,denomination,email) LIKE '%{$rech}%' ORDER BY nom ASC ";
        $req = $bdd->prepare($req);
