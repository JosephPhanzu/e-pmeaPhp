<?php
    ...
    $outPut = "";
    if (isset($_POST['r']) && isset($_POST['id'])) {
        $rech = securisation($_POST['r']);
        $getId = htmlspecialchars(intval($_POST['id']));

        $req = $bdd->prepare('SELECT * FROM recensement WHERE id_user = :id AND CONCAT(nom, postnom, prenom, nature, denomination, email) LIKE :rech   ORDER BY nom ASC');
