<?php
    ...
    if (isset($_GET['id'])) {
        ...
        $req->bindParam(':id', $getId, PDO::PARAM_INT);
        $req->execute();
        $resultat = $req->fetch();
        if ($req->rowCount()>0) {
...
<head>
    ...
    <title>Enregistrement Agent</title>
        ...
        <div class="row d-flex mt-4 d-flex justify-content-center">
            <form action="" id="form-recensement" class="form-inline mt-3 py-3 px-0 bg-light border border-primary">
                <div class="container text-secondary text-wrap">
                    <fieldset>
                        <legend class="text-secondary fw-bolder text-uppercase">Listes d'Enregistrés pour l'agent <?= $resultat['prenom'].' '.$resultat['nom'].' commune de '.$resultat['communeA'] ?></legend>
        ...
        $(document).ready(function(){
            ...
            $('input[type=radio][name=liste]').click(function() {
                ...
                if (valeur.includes("recense")) {
                    ...
                    function recupPat(page) {
                        $.ajax({
                            url : 'ajax/recupPatentA.php',
                            type : 'POST',
                            data : {type : typePat, page : page, id : id},
                            success : function(datas){
                                $('#info').html(datas);
                                totalPages = $('#total-pages').val();
                                totalPages = parseInt(totalPages);
                    ...
                    function recupPatC(page) {
                        $.ajax({
                            url : 'ajax/recupPatentA.php',
                            type : 'POST',
                            data : {type : typePat1, page : page, id : id},
                            success : function(datas){
                                $('#info').html(datas);
                                totalPages = $('#total-pages').val();
                                totalPages = parseInt(totalPages);
            ...
            $(document).on('keyup', '#rechercheRece,#recherchePat', function(){
                ...
                if ($(this).is('#rechercheRece')) {
                    ...
                    if (r != "") {
                        $.ajax({
                            url : 'ajax/recherchePatA.php',
                            type : 'post',
                            data : {r : r, type : type, id : id},
                            success : function(datas){
                                $('#infoR').show();
                                $('#infoR').html(datas);
                                $('#info').hide();
